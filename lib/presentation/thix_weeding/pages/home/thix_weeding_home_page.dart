...
class _P {
  ...
  static const primary = Color(0xFFE93D6D); // bouton Rechercher
...
@riverpod Future<Map<String, int>> homeStats(HomeStatsRef ref) async => {'a':1};
@riverpod
...
@riverpod Future<List<Map<String, dynamic>>> homeAnnouncements(HomeAnnouncementsRef ref) async => [];

class ThixWeedingHomePage extends ConsumerStatefulWidget {
  const ThixWeedingHomePage({super.key});
  @override ConsumerState<ThixWeedingHomePage> createState() => _ThixWeedingHomePageState();
}
...
class _ThixWeedingHomePageState extends ConsumerState<ThixWeedingHomePage> {
  ...
  @override void initState() { super.initState(); _idController = TextEditingController(); _focusNode = FocusNode(); }
  @override void dispose() { _idController.dispose(); _focusNode.dispose(); super.dispose(); }

  Future<void> _onStaffAccess() async {
    final user = Supabase.instance.client.auth.currentUser;
    if (user == null) { ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('Connectez-vous pour accéder à l\'espace staff'))); context.push('/thix-weeding/auth/login'); return; }
    try { final repo = ref.read(weddingRepositoryProvider); final weddings = await repo.getWeddingsByOwnerId(user.id); if (!mounted) return; if (weddings.isEmpty) { context.push('/thix-weeding/create'); } else if (weddings.length == 1) { context.push('/thix-weeding/staff/${weddings.first.id}'); } else { context.push('/thix-weeding/staff/my-weddings'); } } catch (_) { if (!mounted) return; context.push('/thix-weeding/staff/my-weddings'); }
  }

  Future<void> _onSearch() async {
    final id = _idController.text.trim(); if (id.isEmpty) { _focusNode.requestFocus(); return; } if (_isSearching) return;
    setState(() => _isSearching = true); FocusScope.of(context).unfocus();
    try { final repo = ref.read(weddingRepositoryProvider); final WeddingEntity wedding = await repo.getWeddingById(id); if (!mounted) return; context.push('/thix-weeding/guest/${wedding.id}'); } on Failure catch (e) { if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(e.message), backgroundColor: Colors.red.shade600)); } catch (_) { if (!mounted) return; ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('ID introuvable, vérifiez et réessayez'))); } finally { if (mounted) setState(() => _isSearching = false); }
  }
...
class _TopHeader extends StatelessWidget {
  final int notificationCount; final VoidCallback onNotificationsTap; final VoidCallback onProfileTap;
  const _TopHeader({required this.notificationCount, required this.onNotificationsTap, required this.onProfileTap});
  @override Widget build(BuildContext context) {
    return Padding(
...
class _HeroSearchSection extends StatelessWidget {
  final TextEditingController controller; final FocusNode focusNode; final bool isLoading; final VoidCallback onSearch; final VoidCallback onScanQr;
  const _HeroSearchSection({required this.controller, required this.focusNode, required this.isLoading, required this.onSearch, required this.onScanQr});
  @override Widget build(BuildContext context) {
    return Padding(
...
class _CategoryGrid extends StatelessWidget {
  final List<Map<String, dynamic>> categories; final void Function(String) onTap;
  ...
  @override Widget build(BuildContext context) {
    return Container(margin: const EdgeInsets.symmetric(horizontal: 16), padding: const EdgeInsets.fromLTRB(12, 16, 12, 12), decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(20), boxShadow: _P.shadowSoft), child: GridView.builder(shrinkWrap: true, physics: const NeverScrollableScrollPhysics(), itemCount: categories.length, gridDelegate: const SliverGridDelegateWithFixedCrossAxisCount(crossAxisCount: 5, mainAxisSpacing: 14, crossAxisSpacing: 4, childAspectRatio: 0.78), itemBuilder: (context, i) { final c = categories[i]; return InkWell(onTap: () => onTap(c['label']), borderRadius: BorderRadius.circular(12), child: Column(children: [Container(width: 52, height: 52, decoration: BoxDecoration(color: bubbles[i % bubbles.length], borderRadius: BorderRadius.circular(16)), child: Icon(c['icon'] as IconData, size: 24, color: colors[i % colors.length])), const SizedBox(height: 6), Text(c['label'] as String, textAlign: TextAlign.center, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w600, color: _P.ink, height: 1.15))])); }));
  }
}

class _OffersRow extends StatelessWidget {
  final List<Map<String, dynamic>> offers; final void Function(String) onTap;
  const _OffersRow({required this.offers, required this.onTap});
  @override Widget build(BuildContext context) {
    return SizedBox(height: 148, child: ListView.separated(padding: const EdgeInsets.symmetric(horizontal: 16), scrollDirection: Axis.horizontal, itemCount: offers.length, separatorBuilder: (_, __) => const SizedBox(width: 10), itemBuilder: (context, i) { final o = offers[i]; final isBigDiscount = (o['discount'] as String).contains('%'); return InkWell(onTap: () => onTap(o['title']?? o['discount']), borderRadius: BorderRadius.circular(16), child: Container(width: 148, decoration: BoxDecoration(color: o['bg'] as Color, borderRadius: BorderRadius.circular(16)), child: Stack(children: [Padding(padding: const EdgeInsets.all(12), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(o['discount'] as String, style: TextStyle(fontSize: isBigDiscount? 22 : 14, fontWeight: FontWeight.w900, color: o['color'] as Color, height: 1.1)), const SizedBox(height: 4), if ((o['title'] as String).isNotEmpty) Text(o['title'] as String, style: const TextStyle(fontSize: 11, fontWeight: FontWeight.w700, color: _P.ink)), const SizedBox(height: 4), Text(o['subtitle'] as String, style: const TextStyle(fontSize: 9.5, color: _P.inkSoft, height: 1.2))])), Positioned(bottom: 0, left: 0, right: 0, child: ClipRRect(borderRadius: const BorderRadius.vertical(bottom: Radius.circular(16)), child: Image.network(o['image'] as String, height: 64, fit: BoxFit.cover, errorBuilder: (_, __, ___) => const SizedBox(height: 64))))] ))); }));
  }
}

class _ProvidersGrid extends StatelessWidget {
  final List<Map<String, dynamic>> providers; final void Function(String) onTap;
  const _ProvidersGrid({required this.providers, required this.onTap});
  @override Widget build(BuildContext context) {
    return SizedBox(height: 220, child: ListView.separated(padding: const EdgeInsets.symmetric(horizontal: 16), scrollDirection: Axis.horizontal, itemCount: providers.length, separatorBuilder: (_, __) => const SizedBox(width: 12), itemBuilder: (context, i) { final p = providers[i]; return InkWell(onTap: () => onTap(p['name']), borderRadius: BorderRadius.circular(16), child: Container(width: 160, decoration: BoxDecoration(color: Colors.white, borderRadius: BorderRadius.circular(16), boxShadow: _P.shadowSoft), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Stack(children: [ClipRRect(borderRadius: const BorderRadius.vertical(top: Radius.circular(16)), child: Image.network(p['image'] as String, height: 110, width: 160, fit: BoxFit.cover, errorBuilder: (_, __, ___) => Container(height: 110, color: _P.border))), Positioned(top: 8, right: 8, child: Container(width: 28, height: 28, decoration: BoxDecoration(color: Colors.white, shape: BoxShape.circle, boxShadow: _P.shadowSoft), child: const Icon(Icons.favorite_border_rounded, size: 16, color: _P.ink)))]), Padding(padding: const EdgeInsets.all(10), child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [Text(p['name'] as String, maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 12, fontWeight: FontWeight.w800, color: _P.ink)), const SizedBox(height: 2), Text('${p['category']} • ${p['zone']}', maxLines: 1, overflow: TextOverflow.ellipsis, style: const TextStyle(fontSize: 10, color: _P.inkSoft)), const SizedBox(height: 4), Row(children: [const Icon(Icons.star_rounded, size: 12, color: Color(0xFFFFB400)), const SizedBox(width: 2), Text('${p['rating']} (${p['reviews']})', style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: _P.ink))]), const SizedBox(height: 4), Text(p['price'] as String, style: const TextStyle(fontSize: 10, fontWeight: FontWeight.w700, color: Color(0xFF2B6BFF))) ]))]))); }));
  }
}
