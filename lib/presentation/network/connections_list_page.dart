...
class Connections extends AsyncNotifier<List<Map<String, dynamic>>> {
  ...
  Future<List<Map<String, dynamic>>> _fetch(int offset) async {
    ...
    // 1. Mise à jour de la requête pour cibler user1_id et user2_id vers la table profiles
    final res = await supa.from('connections').select('''
      id, created_at, user1_id, user2_id,
      user1:profiles!user1_id (id, display_name, photo_url, profession, bio),
      user2:profiles!user2_id (id, display_name, photo_url, profession, bio)
    ''')
    ...
    for (var conn in res as List) {
      // 3. Identifier qui est "l'autre" utilisateur dans cette connexion
...
class _ConnectionsListPageState extends ConsumerState<ConnectionsListPage> {
  ...
  Future<void> _removeConnection(String connectionId, String userName) async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('$userName a été retiré'), backgroundColor: Colors.green));
      }
    } catch (e) {
      if (mounted) {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
        ...
        title: Row(children: [
          ...
          asyncConnections.when(
            data: (l) => Container(
              ...
              child: Text('${l.length}', style: const TextStyle(color: Color(0xFFD4AF37), fontWeight: FontWeight.bold, fontSize: 12))
            ), 
      ...
      body: asyncConnections.when(
        ...
        data: (connections) => connections.isEmpty ? _buildEmptyWidget() : RefreshIndicator(
          ...
          child: ListView.builder(
            ...
            itemBuilder: (context, index) {
              ...
              return ConnectionCard(
                ...
                onMessageTap: () => context.push('/network/chat/${conn['user_id']}'), 
  ...
  Widget _buildErrorWidget(String error) => Center(
    child: Column(
      ...
      children: [
        ...
        Text(error, style: TextStyle(fontSize: 16, color: Colors.grey.shade600)), 
        const SizedBox(height: 16), 
        ElevatedButton(
    ...
    child: Column(
      ...
      children: [
        Container(
          ...
          decoration: BoxDecoration(color: const Color(0xFFD4AF37).withOpacity(0.08), shape: BoxShape.circle), 
          child: Icon(Icons.people_outline, size: 64, color: const Color(0xFFD4AF37).withOpacity(0.5))
        ), 
        const SizedBox(height: 24), 
        const Text('Aucune connexion', style: TextStyle(fontSize: 20, fontWeight: FontWeight.bold, color: Color(0xFF1A1A2E))), 
        const SizedBox(height: 8), 
        Text('Commencez à vous connecter avec\ndes professionnels de votre secteur.', textAlign: TextAlign.center, style: TextStyle(fontSize: 14, color: Colors.grey.shade600)), 
        const SizedBox(height: 24), 
        ElevatedButton.icon(
          onPressed: () => context.push('/network/discover'), 
          icon: const Icon(Icons.explore), 
          label: const Text('Découvrir des personnes'), 
          style: ElevatedButton.styleFrom(backgroundColor: const Color(0xFFD4AF37), foregroundColor: Colors.white, shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(24)), padding: const EdgeInsets.symmetric(horizontal: 24, vertical: 12))
        )
      ]
    )
  );

  void _showSearch() => showDialog(
    context: context, 
    builder: (c) => AlertDialog(
      ...
      content: TextField(
        ...
        autofocus: true, 
        decoration: const InputDecoration(hintText: 'Nom...', prefixIcon: Icon(Icons.search)), 
