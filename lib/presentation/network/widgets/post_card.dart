...
class PostItemNotifier extends StateNotifier<NetworkPost> {
  ...
  Future<void> toggleLike() async {
    ...
    try {
      ...
      try {
        final result = await service.togglePostLike(state.id);
        state = state.copyWith(
          isLiked: result.liked,
          likesCount: result.likesCount,
        );
      } catch (_) {
        // Fallback anciennes API
        if (wasLiked) {
          await service.unlikePost(state.id);
        } else {
          await service.likePost(state.id);
        }
      }
    } catch (_) {
...
class _PostCardState extends ConsumerState<PostCard>
  ...
  // ── Navigation sûre (évite pile / context invalide) ──
  ...
  void _safePush(String location) {
    ...
    try {
      ...
      debugPrint('nav push failed: $e');
    }
  }

  void _openProfile(String userId) {
    if (userId.isEmpty) return;
    _safePush('/network/profile/$userId');
  }
  ...
  void _openHashtag(String tag) {
    _safePush('/hashtag/$tag');
  }

  // ── Contenu riche (profondeur limitée = anti stack overflow) ──
        ...
        RichText(
          text: TextSpan(
            children: _isExpanded
                ? (_cachedFullSpans ?? const [])
                : (_cachedTruncatedSpans ?? const []),
          ),
        ),
        if (_isTruncatable)
          GestureDetector(
  ...
  // ── Share lien deep-link ──

  Future<void> _sharePost(NetworkPost post) async {
    final link =
        'https://depot-data-thix-central.github.io/sona1/network/post/${post.id}';
    await Clipboard.setData(ClipboardData(text: link));
    try {
      await ref.read(networkServiceProvider).sharePost(post.id);
    } catch (_) {}
  ...
  Future<void> _repost(NetworkPost post) async {
    ...
    try {
      final service = ref.read(networkServiceProvider);
      try {
        await service.repostPost(post.id, quote: _quoteController.text);
      } catch (_) {
        await service.repost(post.id, _quoteController.text);
      }
      if (!mounted) return;
  ...
  Widget _buildImageGrid(List<String> urls, String postId) {
    ...
    Widget img(int i, double h) => GestureDetector(
          onTap: () {
            if (!mounted) return;
          ...
          child: ClipRRect(
            borderRadius: radius,
            child: _buildNetworkImage(urls[i],
    ...
    if (urls.length == 1) {
      return ClipRRect(
        ...
        child: GestureDetector(
          ...
          child: _buildNetworkImage(urls[0], height: 280),
        ),
      );
    }
    ...
    if (urls.length == 2) {
      return SizedBox(
        height: 200,
        child: Row(children: [
          Expanded(child: img(0, 200)),
          const SizedBox(width: spacing),
          Expanded(child: img(1, 200)),
        ]),
      );
    }
    ...
    return SizedBox(
      ...
      child: Row(
        children: [
          Expanded(flex: 3, child: img(0, 240)),
          const SizedBox(width: spacing),
          Expanded(
            flex: 2,
            child: Column(
              children: [
                Expanded(child: img(1, 118)),
                      ...
                      children: [
                        ...
                        PopupMenuButton<String>(
                          icon: const Icon(Icons.more_vert_rounded,
                              size: 18, color: _PostColors.primaryDeep),
                          shape: RoundedRectangleBorder(
                            borderRadius: BorderRadius.circular(16),
                          ),
                          onSelected: (v) async {
                        ...
                        _actionPill(
                          ...
                          onTap: () async {
                            ...
                            Future.delayed(
                              ...
                              () {
                                if (mounted) {
                                  setState(() => _isLikedAnimating = false);
                                }
                              },
                            );
                            widget.onLike?.call();
...
// ─────────────────────────────────────────────────────────────
// GALLERY (navigation simple, pas de stack go_router)
// ─────────────────────────────────────────────────────────────
